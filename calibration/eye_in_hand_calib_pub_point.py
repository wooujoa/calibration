...
class CameraPointBaseTransformer(Node):
    def __init__(self):
        super().__init__('camera_point_base_transformer_left')

        # ------------------------------------------------------------------
        # Camera optical frame -> Left Gripper
        # ------------------------------------------------------------------
        ...
        self.declare_parameter('aruco_input_topic', '/aruco/marker_3d_l')
        self.declare_parameter('yolo_input_topic', '/yolo/target_3d_pose_l')

        self.declare_parameter('aruco_output_topic', '/aruco/marker_base_pose_l')
        self.declare_parameter('yolo_output_topic', '/yolo/target_base_pose_l')
        ...
        self.get_logger().info('Camera Point Base Transformer Initialized (LEFT ARM, OPTICAL INCLUDED)')
    ...
    def aruco_callback(self, msg: PointStamped):
        self.transform_and_publish(msg, self.pub_aruco_base, 'ARUCO-L')

    def yolo_callback(self, msg: PointStamped):
        self.transform_and_publish(msg, self.pub_yolo_base, 'YOLO-L')
